...
class SystemLog
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        switch($path){
            ...
            case 'backstagePage': // 后台首页
                $log_content = $common_content . '进入了后台首页';
                break;
        }
        ...
        if($log_content && $uid){
            ...
            /*$roleRst = RoleUserModel::where('user_id', $uid)->first();
            ($roleRst->role_id) && ($roleInfo = Role::where('id', $roleRst->role_id)->first());
            $user_type = $roleInfo->id ? $roleInfo->id : 0;*/
